...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def prepare_test_frames(self, idx):
        ...
        if self.multi_class and isinstance(results['label'], list):
            ...
            onehot[results['label']] = 1.
            results['label'] = onehot
        retval = self.pipeline(results)
        #retval['classes'] = ';'.join(self.classes)
        return retval
...
class VideoDataset(BaseDataset):
    ...
    def classes(self):
        ...
        with open(self.labels_file, 'r') as f:
            classes_with_id = f.read().split('\n')[1:]
        return [','.join(x.split(',')[1:]) for x in classes_with_id]
...
def build_dataloader(logger, config):
    ...
    return train_data, val_data, train_loader, val_loader, classes
